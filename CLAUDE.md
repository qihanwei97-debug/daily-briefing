...
**严格语义：本表只列"今天/近期可触达使用的 AI 产品 / 工具 / 模型 / API / 开源仓库 / 软件更新"**——目的是让读者扫一眼就知道"我下午能装/能试什么"。
...
**🚫 严禁进入本表的内容（这些归 ② 速报）：**
- 财报 / Q 业绩 / 营收数据（如"AMD Q1 +57%"）
- 诉讼 / 庭审进展（如"Meta 集体诉讼"）
- IPO / 路演 / 上市动态（如"Cerebras 路演"）
- 合作 / 收购 / JV / 战略联盟（如"Nvidia × Corning $500M"）
- 政府采购 / 联邦合约（如"GSA + ChatGPT $1/agency"）
- 人事 / 高管访谈 / 公司治理变动

**判断方法：填到表里之前，问自己"读者能在 5 分钟内点击'获取方式'链接、装上/打开它/调用它吗？"**——
- ✅ 能（产品发布、API 上线、开源仓库、应用更新、模型权重发布）→ 入表
- ❌ 不能（财报、诉讼、合作、IPO、合约）→ 走 ② 速报，不入本表

**可接受 5-8 行**——宁可少而精准，不要凑数把事件硬塞。
