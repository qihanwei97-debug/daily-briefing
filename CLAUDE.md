...
```
🔔 今日关键（60 秒）
- **【事件核心 ≤25 字】**——对法律工作的参考意义（当相关时，1 句，平级语气）
- 法律无关条目：省略 dash 后缀，仅写事件核心
```

**示例：**
- **Meta 2026 AI Capex $125B 翻倍**——做美股客户的同事可参考其估值模型变化
- **Nebraska 首例律师因 AI 幻觉吊销执照**——提醒：AI 起草后的引文人工核对很有必要
...
**头条（1-2条）：** 发生了什么 + 为什么重要（含对法律工作的参考意义，平级语气） + 实用信息（能不能用？怎么用？免费/付费？），引用至少2个来源。

**⚠️ 头条"为什么重要"段落严禁堆砌"汉坤 X 业务组本周必须 / 应该 / 强制 / 全员"等管理层口吻。** 写作视角是"我作为一线律师，看到这条新闻，对哪些同事的工作可能有参考"——而非"所里下周要做什么决议"。

**速报（逐条，按重要性排列）：** 每条2-3行，格式统一：
```
**产品名**（日期，来源）：一句话说明
- 💡 关联：一句话说对法律工作的关联（"做 X 业务的同事可参考……"，可省略）
```
...
**核心定位：信息整合 + 受众相关性提示——不是下命令、也不是个人分享。**

写作目的是"客观汇总今日 AI 动态 + 指出对哪些业务条线的同事可能有参考价值"。**既不**写"汉坤本周必须做 X"（管理层口吻），**也不**写"我自己平时是这样用的"（个人化口吻——AI 没有自己）。
...
- **法律视角增补：** 非法律主题新闻在"为什么重要"段落可补一句"对法律行业的参考意义"（当相关时）——典型映射：
  - 跨国并购/裁员/融资 → 涉外 M&A 同事可关注的尽调点
  - 金融制裁/出口管制 → 合规 / 出口管制同事可关注的新动向
  - AI 新产品 → 律师使用场景、合规审查可关注角度
- **技术细节节制：** 模型架构、训练方法、激活参数等纯技术内容**简化或省略**，让位给参考价值
