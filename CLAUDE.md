...
**头条（1-2条）：** 发生了什么 + 为什么重要（事件本身的行业影响 / 数据冲击 / 趋势含义） + 实用信息（能不能用？怎么用？免费/付费？），引用至少2个来源。

**⚠️ 头条"为什么重要"段落写事件本身在 AI / 科技行业的意义即可——不强加律师视角。** 仅当是"直接法律事件"（AI 幻觉案、AI 法规、AI 反垄断判决、AI 著作权诉讼等）时，法律含义才作为段落核心展开。其他类型新闻（IPO、融资、模型发布、benchmark、GitHub Trending 等）写完事件影响就结束，不再补"对汉坤 X 业务的影响"。严禁管理层口吻和第一人称。
...
1. **直接法律 / 合规事件：** AI 幻觉案罚单、AI 监管法规、AI 反垄断判决、AI 著作权诉讼等——这种内容的法律含义是新闻本身的核心，可以自然展开。
2. **AI 工具 / 实用技巧（板块 ⑤）：** 当技巧确实对法律工作有具体应用场景时，可点明"做 X 业务时可以这样用"。

像 IPO 估值、芯片融资、GitHub Trending、模型 benchmark、产品发布这类纯科技 / 商业新闻——**不再硬塞律师视角**。读者是律师，自己会判断关联。
