...
```
# AI 日报
**YYYY年M月D日 星期X** | 覆盖XX小时 | 预计阅读10-12分钟
```
...
**示例：**
- **Trump 无限期延长伊朗停火**——航空/能源 M&A 尽调需留意价格波动
...
**头条（1-2条）：** 发生了什么 + 为什么重要 + 技术细节 + 实用信息（能不能用？怎么用？免费/付费？），引用至少2个来源。

**速报（逐条，按重要性排列）：** 每条3-5行，格式统一：
```
**产品名**（日期，来源）：一句话说明
- 📍 获取：URL/命令/操作路径
- 💰 定价：免费 / 付费 / 开源
...
2-3条最重要的非AI新闻，每条3-5行。侧重地缘政治、经济政策、与科技相关事件。
...
**旗舰模型对比表：**
- 固定对比：Anthropic旗舰、OpenAI旗舰、Google旗舰（这三家必须纳入）
- 动态对比：根据搜索结果，选取当期最具竞争力的1-2个其他旗舰模型（如DeepSeek、Qwen、Mistral、xAI等，视当期表现和热度决定）
- 具体型号以搜索结果为准，不硬编码
- **维度（8 个，律所受众适配）：** 综合推理、办公文档集成、设计/可视化、中文能力、长上下文(实际可用)、定价(input+output/M)、最佳适用、获取方式
- **必须使用⭐星级评分**（如 ⭐⭐⭐⭐⭐、⭐⭐⭐⭐½），数值型指标同时标注具体数字
- 已砍掉的技术维度（编程/Agentic编程/算法科学推理/网络安全/最大输出）**仅在当期新模型发布且该项为最大亮点时**作为一次性备注出现在表后点评中，不占列

**中端性价比模型对比表：**
- 固定对比：Anthropic中端、OpenAI中端、Google中端（这三家必须纳入）
- 动态对比：根据搜索结果，选取当期最具性价比优势的1-2个其他中端模型
- 重点突出性价比差异，同样使用⭐星级评分

**表后必须附：** "📝 本周变化"点评（1-3句，指出排名/数据变动）

**无变化时的处理：** 如果本周无重大模型发布，可标注"本周数据与上期一致，无重大排名变动"。

### ⑥ 场景化AI工具推荐（每期必含，13个固定场景）

表格必须为五列：`| 场景 | 首选 | 备选 | 免费方案 | 一句话理由 |`
...
推荐原则：首选=综合最佳（不考虑价格），备选=接近首选或子场景更优，免费方案=零成本或开源，一句话理由≤15字，排名变化用🔄标注。
